...
this high-level reference I cover the tools that I find most
...
available theorem environments and their respective keywords.
...
below) with a unique label (containing no spaces) that will be convenient
to reference.
...
Notice that definition's number was automatically incremented from the
previous definition.
...
The `\left(` / `\right)` command encloses its contents in big brackets.
...
The `~` symbol represents a whitespace character.
...
placing `&&` followed by `(<my LaTeX-formatted comment>)` at the end of the line,
before the number label. As with our use of the `&` operator to align
successive lines of math, the `&&` operator tells the LaTeX
compiler to align comments, but to do so to the right of the 
aligned math.
...
We use `scriptstyle` to make the inline text smaller. Other custom
display options are detailed
...
A good reference for theme options is
...
Use the knitr option `layout="l-body-outset"` to make the figure or
table span the width of the page. More information
...
The `"class=external"` knitr option creates a frame around the image.
...
| Example                          | Raw                               |
+==================================+===================================+
| *Bookdown environment*: See      | `@thm-pyth`                       |
| Theorem @thm-pyth.               |                                   |
+----------------------------------+-----------------------------------+
| *Math equation*: See equation    | `@eq-var-beta`                    |
| @eq-var-beta.                    |                                   |
+----------------------------------+-----------------------------------+
| *Figure or image*: See Fig.      | `@fig-1`                          |
| @fig-1.                          |                                   |
+----------------------------------+-----------------------------------+
...
Table 2.1 on
[this](https://bookdown.org/yihui/bookdown/markdown-extensions-by-bookdown.html)
page from *Authoring Books with R Markdown* lists the label prefixes for all other
available theorem environments (we use the `thm` prefix here).
